"""Compare VIF vs Lasso-LARS vs forward-stepwise region menus."""
...
def make_synthetic(n_per_region: int = 300, p: int = 20, seed: int = 0):
    ...
    for rid, center in enumerate(centers):
        ...
        Xs.append(X)
        ys.append(y)
    return np.vstack(Xs), np.concatenate(ys), true_supports
...
def run_dataset(name: str, X, y, *, n_regions: int = 3, seed: int = 0) -> None:
    ...
    # Same region geometry; swap only the selector.
    base = {
        "n_regions": n_regions,
        "partition": "residual",
        "assignment": "soft",
    ...
    policies = {
        "vif": {**base, "selector": "vif"},
        "lasso_lars": {**base, "selector": "lasso_lars"},
        "forward_stepwise": {**base, "selector": "forward_stepwise"},
    }

    probe = fit_region_menus(X_train, y_train, random_state=seed, **policies["vif"])
    max_b = max(max(len(m.selected) for m in probe.menus), 5)
    # Lasso/forward may select more; pad budgets a bit.
    for key in ("lasso_lars", "forward_stepwise"):
        m = fit_region_menus(X_train, y_train, random_state=seed, **policies[key])
        max_b = max(max_b, max(len(r.selected) for r in m.menus))
    ...
    print("\nRMSE vs feature budget (residual/soft regions)")
    print(format_curves_table(curves))

    for sel in ("vif", "lasso_lars", "forward_stepwise"):
        model = fit_region_menus(X_train, y_train, random_state=seed, **policies[sel])
        print(f"\nMenus [{sel}]:")
    ...
    for b in (mid, budgets[-1]):
        ...
        ranked.sort()
        print(
            f"\nAt budget={b}, best RMSE: "
            + ", ".join(f"{n}={r:.4f}" for r, n in ranked[:4])
        )


def main() -> None:
    print("Comparing selectors: vif | lasso_lars | forward_stepwise")

    X_s, y_s, _ = make_synthetic()
